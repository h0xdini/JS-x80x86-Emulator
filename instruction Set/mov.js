  const movInst = (source, destination, hexTwice) => {
    ...
    if (destination.toLowerCase() === 'ax') {
      ...
      const base = destination.replaceAll('[', '').replaceAll(']', '').includes('0x') ? 16 : destination.replaceAll('[', '').replaceAll(']', '').includes('b') ? 2 : 10
      let memoryPosition

      if (!destination.includes('b')) {
        memoryPosition = parseInt(destination.replaceAll('[', '').replaceAll(']', ''), base)
      } else {
        memoryPosition = parseInt(destination.replaceAll('[', '').replaceAll(']', '').replaceAll('b', ''), base)
      }
      if (source <= 0xFFFF) {
